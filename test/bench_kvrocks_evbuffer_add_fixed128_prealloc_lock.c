 ...
 * each batch's destination chain and enabling an evbuffer pthread lock.
 */
...
main(int argc, char **argv)
{
	...
	for (done = 0; ops == 0 || done < ops; done += todo) {
		...
		if (evbuffer_enable_locking(buf, NULL) < 0) {
			fprintf(stderr, "evbuffer_enable_locking failed\n");
			evbuffer_free(buf);
			free(value);
			return 1;
		}
	...
	printf("bench=evbuffer_add_fixed128_prealloc_lock ops=%ld bytes="
	    U64_FMT " usec=%ld ops_sec=%.2f mb_sec=%.2f\n",
