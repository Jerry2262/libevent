 ...
 * each batch's destination chain and installing one enabled callback.
 */
...
static volatile bench_u64 callback_sink;
...
static void
on_buffer_change(struct evbuffer *buffer, const struct evbuffer_cb_info *info,
    void *arg)
{
	(void)buffer;
	(void)arg;
	callback_sink += (bench_u64)info->n_added;
}
...
main(int argc, char **argv)
{
	...
	for (done = 0; ops == 0 || done < ops; done += todo) {
		...
		if (evbuffer_add_cb(buf, on_buffer_change, NULL) == NULL) {
	...
	printf("bench=evbuffer_add_fixed128_prealloc_cb ops=%ld bytes=" U64_FMT
